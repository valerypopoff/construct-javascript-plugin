var __CONSTRUCT3PRESENTED__ = true;
var __DEBUG__ = false;
...
(function ()
{
	...
	} else
	{
		...
		{	
			...
			//Calling the CallJSfunction action with the flag "false" so it doesn't not store return value. 
			//We only want to store return value when the user explicitly calls Plugin Actions 
			ret = pluginProto.acts.CallJSfunction.call( this, funcname_, funcparams_, store_return_value_, "'Compare Function return value' condition" );
	...
	exps.JSCodeValue = function (ret, code_)	
	{
		...
		//Then delete first two params: ret and code_ leaving only parameters that were passed to a code (if any)
